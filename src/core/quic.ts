/**
 * Real QUIC Initial packet generator for the AmneziaWG `I1` masking parameter.
 *
 * This is the modern upgrade over the old server's `buildQUICInitialPacket`,
 * which emitted a *malformed* QUIC packet (plaintext ClientHello, fake packet
 * number, no header protection). Here we build a genuinely valid QUIC Initial:
 * derive the initial secrets (HKDF via HMAC-SHA256), AES-GCM encrypt the CRYPTO
 * frame carrying a TLS ClientHello (SNI only), and apply header protection.
 * A well-formed QUIC handshake is what current DPI expects, so it passes far
 * better than the old fake.
 *
 * Faithful port of nellimonix/warp-config-generator-vercel `lib/quic.ts`
 * (itself a port of warp-generator.github.io quic.js). WebCrypto — AES-GCM,
 * AES-CBC, HMAC-SHA256, getRandomValues — is available in the Tauri webview.
 */

// BufferSource (ArrayBuffer | ArrayBufferView) — what WebCrypto accepts.
...
function quicVarintLength(x: number): number {
  if (x < 0x40) return 1;
  else if (x < 0x4000) return 2;
  else if (x < 0x40000000) return 4;
  else return 8;
}
...
async function quicInitHmacKey(): Promise<void> {
  ...
  quicHmacKey = await globalThis.crypto.subtle.importKey("raw", quicSalt, { name: "HMAC", hash: "SHA-256" }, false, [
    "sign",
  ]);
}

async function quicDeriveSecret(
...
async function quicEncryptPayload(
  ...
  const cryptoKey =
    key instanceof CryptoKey
      ? key
      : await globalThis.crypto.subtle.importKey("raw", key, { name: "AES-GCM", length: 128 }, false, ["encrypt"]);
...
async function quicDeriveHpMask(key: CryptoKey | ArrayBuffer, sample: BufferSource): Promise<ArrayBuffer> {
  // ECB is not implemented in Subtle, so we use a single block with a zero IV.
  const cryptoKey =
    key instanceof CryptoKey
      ? key
      : await globalThis.crypto.subtle.importKey("raw", key, { name: "AES-CBC", length: 128 }, false, ["encrypt"]);
...
function quicMeasureLengths(
  ...
  if (overallLength < padto) {
    ...
    // Adjust padding down if it grew due to a bigger length byte.
    while (paddingLength && getOverallLength() > padto) {
      paddingLength--;
    }
    // It can underflow by one; nudge back.
    if (getOverallLength() < padto) {
      paddingLength++;
    }
    overallLength = getOverallLength();
  }
  // Extra safety: the tail from pkn must be at least 20 bytes for hp key derivation.
...
async function quicInitial(
  dcid: Bytes,
  scid: Bytes,
  token: Bytes,
  pkn: Uint8Array<ArrayBuffer>,
  payload: ArrayBuffer,
  padto: number,
): Promise<ArrayBuffer> {
...
function quicFixCutSettings(cutSettings: number[], packetLength: number, pknLength: number, payloadLength: number): void {
  // Fix the first chunk if it is too short for header protection.
  if (cutSettings[0] < 20 - pknLength) {
    const toAdd = 20 - pknLength - cutSettings[0];
    cutSettings[0] += toAdd;
    cutSettings[1] -= toAdd;
  }
  // Include the header in the first chunk (assumes padto is zero).
  cutSettings[0] += packetLength - payloadLength - 16;
}

/** Generates a valid QUIC Initial packet for `domain`, hex-encoded as `<b 0x...>`. */
export async function generateQuicMask(domain: string): Promise<string> {
...
/** Full `I1 = <b 0x...>` line for the given SNI domain. */
export async function generateI1Line(domain: string): Promise<string> {
  return `I1 = ${await generateQuicMask(domain)}`;
}
